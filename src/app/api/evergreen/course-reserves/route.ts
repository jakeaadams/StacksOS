...
export async function GET(req: NextRequest) {
  try {
    ...
    const materialsRows = Array.isArray(materialsRes?.payload?.[0])
      ? (materialsRes.payload[0] as Record<string, any>[])
      : [];
    ...
    const coursesRaw = Array.isArray(coursesRes?.payload?.[0])
      ? (coursesRes.payload[0] as Record<string, any>[])
      : [];
    const termsRaw = Array.isArray(termsRes?.payload?.[0])
      ? (termsRes.payload[0] as Record<string, any>[])
      : [];

    const courses = coursesRaw
      .map((row) => {
        const owningObj =
          row?.owning_lib && typeof row.owning_lib === "object"
            ? (row.owning_lib as Record<string, any>)
            : null;
        ...
        return {
          ...
          owningLibId,
          owningLibName: owningObj
            ? toString(owningObj.shortname || owningObj.name || "").trim()
            : null,
