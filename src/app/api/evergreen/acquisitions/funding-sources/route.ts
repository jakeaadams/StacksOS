...
const fundingSourcesPostSchema = z
  .object({
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    logger.debug(
      {
        route: "api.evergreen.acquisitions.funding-sources",
        id,
    ...
    if (id) {
      ...
      const response = await callOpenSRF("open-ils.acq", "open-ils.acq.funding_source.retrieve", [
        authtoken,
        sourceId,
        { flesh: 1, flesh_fields: { acqfs: ["owner", "currency_type"] } },
      ]);
      ...
      try {
        const creditsResponse = await callOpenSRF(
          "open-ils.acq",
          "open-ils.acq.funding_source_credit.search",
          [authtoken, { funding_source: sourceId }]
        );
        ...
        creditTotal = (Array.isArray(creditsList) ? creditsList : [])
          .filter(
            (c: unknown): c is CreditRecord =>
              c !== null &&
              typeof c === "object" &&
              !("ilsevent" in c && (c as CreditRecord).ilsevent)
          )
      ...
      try {
        ...
        allocatedTotal = (Array.isArray(allocList) ? allocList : [])
          ...
          .reduce(
            (sum: number, a: AllocationRecord) => sum + (parseFloat(String(a.amount)) || 0),
            0
          );
      } catch {
      ...
      return successResponse({
        fundingSource: {
          ...
          ownerName:
            typeof source.owner === "object" ? source.owner?.shortname || source.owner?.name : null,
    ...
    const mappedSources = await Promise.all(
      (Array.isArray(sources) ? sources : [])
        .filter(
          (s: unknown): s is FundingSourceRecord =>
            s !== null &&
            typeof s === "object" &&
            !("ilsevent" in s && (s as FundingSourceRecord).ilsevent)
        )
        .map(async (s: FundingSourceRecord) => {
          ...
          try {
            const creditsResponse = await callOpenSRF(
              "open-ils.acq",
              "open-ils.acq.funding_source_credit.search",
              [authtoken, { funding_source: s.id }]
            );
            const creditsPayload = creditsResponse?.payload || [];
            const creditsList = Array.isArray(creditsPayload?.[0])
              ? creditsPayload[0]
              : creditsPayload;
            creditTotal = (Array.isArray(creditsList) ? creditsList : [])
              .filter(
              ...
              .reduce(
                (sum: number, c: CreditRecord) => sum + (parseFloat(String(c.amount)) || 0),
                0
              );
          } catch {
            // skip
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action } = body;
    logger.debug(
      { route: "api.evergreen.acquisitions.funding-sources", action },
      "Funding Sources POST"
    );
    ...
    switch (action) {
      ...
      case "update": {
        ...
        const getResponse = await callOpenSRF(
          "open-ils.acq",
          "open-ils.acq.funding_source.retrieve",
          [authtoken, parseInt(String(id), 10)]
        );
        ...
        const response = await callOpenSRF("open-ils.acq", "open-ils.acq.funding_source.update", [
          authtoken,
          existing,
        ]);
        const result = response?.payload?.[0];
      ...
      case "delete": {
        ...
        const response = await callOpenSRF("open-ils.acq", "open-ils.acq.funding_source.delete", [
          authtoken,
