                    ...
                    action={
                      patron?.barcode
                        ? {
                            ...
                            onClick: () =>
                              router.push(
                                `/staff/circulation/checkout?patron=${encodeURIComponent(patron.barcode)}`
                              ),
                          }
                        : undefined
                    }
                    ...
                    action={
                      patron?.barcode
                        ? {
                            ...
                            onClick: () =>
                              router.push(
                                `/staff/circulation/holds-management?patron=${encodeURIComponent(patron.barcode)}`
                              ),
                          }
                        : undefined
                    }
                  ...
                  action={
                    patron?.barcode
                      ? {
                          ...
                          onClick: () =>
                            router.push(
                              `/staff/circulation/bills?patron=${encodeURIComponent(patron.barcode)}`
                            ),
                        }
                      : undefined
                  }
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <div>
              ...
              <CardDescription>
                Blocks prevent certain actions like checkout or holds
              </CardDescription>
            </div>
            <Button size="sm" onClick={() => props.onSetBlockDialogOpen(true)}>
              ...
              <div className="space-y-3">
                {penalties.map((penalty: any, idx: number) => {
                  ...
                  return (
                    <div
                      key={penaltyId || idx}
                      className="flex items-start justify-between p-3 rounded-lg border bg-destructive/5 border-destructive/20"
                    >
                      <div className="flex gap-3">
                        <AlertTriangle className="h-5 w-5 text-destructive mt-0.5" />
                        <div>
                          <div className="font-medium text-destructive">
                            {penaltyInfo?.label || `Penalty #${penaltyTypeId}`}
                          </div>
                          {note && <p className="text-sm text-muted-foreground mt-1">{note}</p>}
                          <p className="text-xs text-muted-foreground mt-1">
                            Added {toDateLabel(setDate)}
                          </p>
                        </div>
                      </div>
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => props.onRemoveBlock(penaltyId)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
      ...
      <TabsContent value="notes" className="mt-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            ...
            <Button size="sm" onClick={() => props.onSetNoteDialogOpen(true)}>
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Created {toDateLabel(note.createDate)}
                        </p>
                      </div>
                    </div>
                    <Button
