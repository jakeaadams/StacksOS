...
export default function EDIPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [accountsJson, messagesJson, typesJson, vendorsJson] = await Promise.all([
        accountsRes.json(),
        messagesRes.json(),
        typesRes.json(),
        vendorsRes.json(),
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge className={(styles[status] || "bg-muted/50") + " flex items-center gap-1"}>
        {icons[status]}
        {status}
      </Badge>
    );
  ...
  const handleSaveAccount = async () => {
    ...
    try {
      ...
      const res = await fetchWithAuth("/api/evergreen/acquisitions/edi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, ...editingAccount }),
      });
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const res = await fetchWithAuth("/api/evergreen/acquisitions/edi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "delete_account", id: selectedAccount.id }),
      });
  ...
  const handleProcessInbound = async (accountId: number) => {
    ...
    try {
      const res = await fetchWithAuth("/api/evergreen/acquisitions/edi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "process_inbound", accountId }),
      });
      const data = await res.json();
      if (data.ok) {
  ...
  const handleRetryMessage = async (messageId: number) => {
    ...
    try {
      const res = await fetchWithAuth("/api/evergreen/acquisitions/edi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "retry_message", messageId }),
      });
      if ((await res.json()).ok) {
  ...
  const filteredMessages = useMemo(() => {
    ...
    if (filterMessageType !== "all")
      filtered = filtered.filter((m) => m.messageType === filterMessageType);
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (m) => String(m.id).includes(q) || m.messageType.toLowerCase().includes(q)
      );
    }
    return filtered;
    ...
    <PageContainer>
      ...
      <PageContent className="p-0">
        <div className="h-full flex flex-col -m-6">
          <div className="bg-muted/50 border-b px-4 py-2 flex items-center gap-2">
            ...
            <Button onClick={() => loadData()} size="sm" variant="outline" disabled={actionLoading}>
            ...
            <div className="relative">
              ...
              <Input
                ...
                aria-label="Search EDI accounts"
              />
            </div>
          ...
          <div className="flex-1 p-4 overflow-auto">
            <Tabs defaultValue="accounts" className="h-full flex flex-col">
                    ...
                    <CardContent className="p-0">
                      {accounts.length === 0 ? (
                        <div className="p-8">
                          <EmptyState
                            icon={Server}
                            title="No EDI accounts"
                            description="Configure EDI accounts to exchange data with vendors."
                            action={{
                              label: "Add Account",
                              onClick: () => {
                                setEditingAccount({});
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              <TableHead>Account / Host</TableHead>
                          ...
                          <TableBody>
                            {accounts.map((a) => (
                              <TableRow
                                ...
                                className={
                                  "cursor-pointer " +
                                  (selectedAccount?.id === a.id ? "bg-muted" : "hover:bg-muted/50")
                                }
                                onClick={() => setSelectedAccount(a)}
                              >
                                <TableCell>
                                  <div className="font-medium">{a.label}</div>
                                  <div className="text-sm text-muted-foreground">{a.host}</div>
                                </TableCell>
                                <TableCell>{getVendorName(a.vendorId)}</TableCell>
                                <TableCell>
                                  {a.lastActivity
                    ...
                    <CardContent>
                      {selectedAccount ? (
                        <div className="space-y-4">
                          <div>
                            <h3 className="font-medium text-lg">{selectedAccount.label}</h3>
                            <Badge variant="outline">ID: {selectedAccount.id}</Badge>
                          </div>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Host:</span>
                              <span className="font-mono">{selectedAccount.host}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Vendor:</span>
                              <span>{getVendorName(selectedAccount.vendorId)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Protocol:</span>
                              <span>{selectedAccount.useHttp ? "HTTP" : "FTP/SFTP"}</span>
                            </div>
                          </div>
                    ...
                    {filteredMessages.length === 0 ? (
                      <div className="p-8">
                        <EmptyState
                          icon={Mail}
                          title="No messages"
                          description="No EDI messages match your filters."
                        />
                      </div>
                    ) : (
                      <Table>
                        ...
                        <TableBody>
                          {filteredMessages.map((m) => (
                            ...
                            >
                              <TableCell>{getDirectionIcon(m.direction)}</TableCell>
                              <TableCell>
                                <div className="font-mono">{m.id}</div>
                                <div className="text-sm text-muted-foreground">{m.messageType}</div>
                              </TableCell>
                              <TableCell>
                                {accounts.find((a) => a.id === m.accountId)?.label ||
                                  "Account " + m.accountId}
                              </TableCell>
                              <TableCell>
                                {m.purchaseOrderId && <span>PO: {m.purchaseOrderId}</span>}
                                {m.invoiceId && <span>Inv: {m.invoiceId}</span>}
                              </TableCell>
                              <TableCell>{new Date(m.createTime).toLocaleDateString()}</TableCell>
              ...
              <TabsContent value="types" className="flex-1 mt-4">
                <Card>
                  <CardHeader className="py-3">
                    <CardTitle className="text-base">EDI Message Types</CardTitle>
                  </CardHeader>
                  <CardContent className="p-0">
                    <Table>
                      <TableHeader>
          ...
          <div className="bg-muted/50 border-t px-4 py-1 text-xs text-muted-foreground flex items-center gap-4">
            ...
            <span>Errors: {messages.filter((m) => m.status === "error").length}</span>
            <div className="flex-1" />
      ...
      <Dialog open={showAccountDialog} onOpenChange={setShowAccountDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingAccount?.id ? "Edit EDI Account" : "Add EDI Account"}</DialogTitle>
            <DialogDescription>
              Configure connection settings for EDI data exchange.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEditingAccount((p) => ({ ...p, label: e.target.value }))}
                placeholder="e.g., Baker and Taylor EDI"
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setEditingAccount((p) => ({ ...p, host: e.target.value }))}
                placeholder="e.g., ftp.vendor.com"
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditingAccount((p) => ({ ...p, username: e.target.value }))}
                />
              </div>
              <div>
                ...
                <Input
                  id="password"
                  type="password"
                  placeholder={editingAccount?.id ? "(unchanged)" : ""}
                  onChange={(e) => setEditingAccount((p) => ({ ...p, password: e.target.value }))}
                />
              </div>
            </div>
            <div>
              ...
              <Select
                id="vendor"
                value={String(editingAccount?.vendorId || "")}
                onValueChange={(v) => setEditingAccount((p) => ({ ...p, vendorId: parseInt(v) }))}
              >
                <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="inbound-dir">Inbound Dir</Label>
                <Input
                  id="inbound-dir"
                  value={editingAccount?.inDirectory || ""}
                  onChange={(e) =>
                    setEditingAccount((p) => ({ ...p, inDirectory: e.target.value }))
                  }
                  placeholder="/incoming"
                />
              </div>
              <div>
                <Label htmlFor="outbound-dir">Outbound Dir</Label>
                <Input
                  id="outbound-dir"
                  value={editingAccount?.outDirectory || ""}
                  onChange={(e) =>
                    setEditingAccount((p) => ({ ...p, outDirectory: e.target.value }))
                  }
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveAccount} disabled={actionLoading}>
      ...
      <Dialog open={showMessageDialog} onOpenChange={setShowMessageDialog}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-auto">
          <DialogHeader>
            <DialogTitle>EDI Message Details</DialogTitle>
          </DialogHeader>
          {selectedMessage && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-mono">{selectedMessage.id}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Type:</span>
                  <span className="ml-2 font-mono">{selectedMessage.messageType}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Direction:</span>
                  <span className="ml-2 capitalize">{selectedMessage.direction}</span>
                </div>
                <div>
                ...
                {selectedMessage.processTime && (
                  <div>
                    <span className="text-muted-foreground">Processed:</span>
                    <span className="ml-2">
                      {new Date(selectedMessage.processTime).toLocaleString()}
                    </span>
                  </div>
                )}
              </div>
              ...
              {selectedMessage.content && (
                <div>
                  ...
                  <pre className="mt-2 p-3 bg-muted rounded-md text-xs font-mono overflow-x-auto max-h-64">
                    {selectedMessage.content}
                  </pre>
                </div>
              )}
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete EDI Account</DialogTitle>
