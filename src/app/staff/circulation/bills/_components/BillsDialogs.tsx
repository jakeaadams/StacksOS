        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {props.selectedCount > 0 ? props.selectedCount : props.outstandingCount} Evergreen
              transaction(s).
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Receipt or drawer note"
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => props.setPaymentDialogOpen(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={props.refundDialogOpen} onOpenChange={props.setRefundDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {props.refundTarget
                ? `Apply a refund against Evergreen transaction ${props.refundTarget.xactId}. The amount cannot exceed the paid total.`
                : "Select a transaction to refund."}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label htmlFor="amount-2" className="text-sm font-medium">
                Amount
              </label>
              <div className="relative">
                <DollarSign className="absolute left-4 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input
                  ...
                  onChange={(e) => props.setRefundAmount(e.target.value)}
                  className="pl-12 text-lg font-mono"
                  placeholder="0.00"
                />
              </div>
              {props.refundTarget && (
                <p className="text-xs text-muted-foreground">
                  Paid: {formatCurrency(props.refundTarget.paid)}
                </p>
              )}
            </div>
            <div className="space-y-2">
              <label htmlFor="note-2" className="text-sm font-medium">
                Note (optional)
              </label>
              <Input
                id="note-2"
                value={props.refundNote}
                onChange={(e) => props.setRefundNote(e.target.value)}
                placeholder="Refund reason or staff note"
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => props.setRefundDialogOpen(false)}>
              Cancel
            </Button>
            <Button
              onClick={props.onProcessRefund}
              disabled={props.isLoading || !props.refundTarget}
            >
              {props.isLoading ? (
                <Loader2 className="h-4 w-4 mr-1 animate-spin" />
              ) : (
