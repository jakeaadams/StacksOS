...
export function PermissionsDialogs(props: PermissionsDialogsProps) {
  ...
  return (
    <>
      <Dialog open={props.groupDialogOpen} onOpenChange={props.setGroupDialogOpen}>
        <DialogContent className="sm:max-w-[560px]">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => props.setGroupDialogOpen(false)}
              disabled={isMutating}
            >
              Cancel
            </Button>
            ...
            >
              {props.groupDialogMode === "create" ? "Create" : "Save"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog
        open={props.mappingDialogOpen}
        onOpenChange={(open) => {
          props.setMappingDialogOpen(open);
          if (!open) props.setPermPickerOpen(false);
        }}
      >
        <DialogContent className="sm:max-w-[560px]">
          <DialogHeader>
            <DialogTitle>
              {props.mappingDialogMode === "add"
              ...
              <div className="rounded-xl border border-border/70 bg-muted/30 px-3 py-2 text-sm">
                {props.selectedGroup?.name ? (
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium">{props.selectedGroup.name}</span>
              ...
              {props.mappingDialogMode === "edit" ? (
                <div className="rounded-xl border border-border/70 bg-muted/30 px-3 py-2 text-sm">
                  <div className="font-mono text-xs">
                    {props.selectedPerm?.code || `perm #${mappingDraft.permId ?? "\u2014"}`}
                  </div>
                  {props.selectedPerm?.description ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    {props.selectedPerm ? (
                      <>
                        <div className="font-mono text-xs truncate">{props.selectedPerm.code}</div>
                        {props.selectedPerm.description ? (
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => props.setPermPickerOpen(!props.permPickerOpen)}
                    disabled={
                      isMutating ||
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setMappingDraft((d) => ({
                    ...d,
                    depth: Number.isFinite(parseInt(e.target.value, 10))
              ...
              />
              <div className="text-xs text-muted-foreground">
                Use <span className="font-mono text-xs">0</span> for default depth. Evergreen uses
                this for scoping.
              </div>
            </div>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => props.setMappingDialogOpen(false)}
              disabled={isMutating}
            >
              Cancel
            </Button>
            <Button
              ...
              disabled={
                isMutating ||
                !canEdit ||
                (props.mappingDialogMode === "add" && !mappingDraft.permId)
              }
            >
              {props.mappingDialogMode === "add" ? "Add" : "Save"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <div className="text-sm">
          <div className="font-mono text-xs">
            {props.removeTarget?.permCode || `perm #${props.removeTarget?.perm ?? "\u2014"}`}
          </div>
          <div className="mt-1 text-xs text-muted-foreground">
            Group:{" "}
            {props.selectedGroup?.name
              ? `${props.selectedGroup.name} (#${props.selectedGroup.id})`
              : props.selectedGroupId
                ? `#${props.selectedGroupId}`
                : "\u2014"}
          </div>
