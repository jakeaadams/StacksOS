  ...
  const loadMetadata = useCallback(async () => {
    ...
    try {
      ...
      const modifiers = Array.isArray(modsJson?.modifiers)
        ? (modsJson.modifiers as Record<string, any>[])
            .map((row) => ({
      ...
      const groups = Array.isArray(groupsJson?.groups)
        ? (groupsJson.groups as Record<string, any>[])
            .map((row) => ({
      ...
      const copyCatsRaw = Array.isArray(catsJson?.copyCategories)
        ? (catsJson.copyCategories as Record<string, any>[])
        : [];
      ...
      const entryRequests = copyCatsRaw
        .map((cat) => {
          ...
          return fetchWithAuth(`/api/evergreen/stat-categories/entries?kind=copy&statCatId=${id}`)
            .then((r) => r.json())
            .then((json) => ({ id, json }))
            .catch(() => ({ id, json: null }));
        })
      ...
      for (const result of entryResponses) {
        const entries = Array.isArray(result?.json?.entries)
          ? (result.json.entries as Record<string, any>[])
              .map((entry) => ({
                id: Number.parseInt(String(entry?.id ?? ""), 10),
                value: String(entry?.value || "").trim(),
              }))
              .filter(
                (entry) => Number.isFinite(entry.id) && entry.id > 0 && entry.value.length > 0
              )
          : [];
    ...
    try {
      const res = await fetchWithAuth(`/api/evergreen/items/${item.id}`, {
        ...
        body: JSON.stringify({
          ...
          floating:
            editFloatingGroupId === "none" ? null : Number.parseInt(editFloatingGroupId, 10),
      ...
      <PageContainer>
        {(() => {
          const breadcrumbLabel =
            item.title && item.title.length > 42
            ...
            <PageHeader
              ...
              actions={[
                  ...
                  : []),
                {
                  label: "View Record",
                  onClick: () =>
                    item.recordId && router.push(`/staff/catalog/record/${item.recordId}`),
                  icon: BookOpen,
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`h-16 w-16 rounded-xl flex items-center justify-center ${getStatusColor(item.statusId)}`}
                  >
                ...
                {isEditing ? (
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        id="circ-modifier"
                        value={editCircModifier || "none"}
                        onValueChange={(value) =>
                          setEditCircModifier(value === "none" ? "" : value)
                        }
                      >
                        <SelectTrigger>
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Select
                          id="loan-duration"
                          value={editLoanDuration}
                          onValueChange={setEditLoanDuration}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          id="fine-level"
                          value={editFineLevel}
                          onValueChange={setEditFineLevel}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        id="floating-group"
                        value={editFloatingGroupId}
                        onValueChange={setEditFloatingGroupId}
                      >
                        <SelectTrigger>
                        ...
                        {copyStatCategories.map((cat) => (
                          <div key={cat.id} className="space-y-2">
                            ...
                            <Select
                              id="cat-name"
                              value={
                                editStatEntries[cat.id] ? String(editStatEntries[cat.id]) : "none"
                              }
                              onValueChange={(value) => {
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        id="holdable"
                        checked={editHoldable}
                        onCheckedChange={setEditHoldable}
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        id="circulates"
                        checked={editCirculate}
                        onCheckedChange={setEditCirculate}
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        id="opac-visible"
                        checked={editOpacVisible}
                        onCheckedChange={setEditOpacVisible}
                      />
                    </div>
                  </>
                ) : (
              ...
              <div className="flex flex-col md:flex-row items-start gap-6">
                {(() => {
                  ...
                  return (
                    ...
                    >
                      {!coverUrl ? (
                        <div className="h-full w-full bg-gradient-to-br from-slate-700 via-slate-600 to-zinc-700 text-white p-3 flex flex-col justify-between">
                          <span className="text-[9px] uppercase tracking-[0.15em] text-white/70">
                            StacksOS
                          </span>
                          <div>
                            <span className="text-3xl font-semibold leading-none">
                              {(item.title || "Book").trim().charAt(0).toUpperCase() || "B"}
                            </span>
                            <p className="mt-2 text-[11px] font-medium leading-snug line-clamp-4 text-white/90">
                              {(item.title || "No title").trim()}
                            </p>
                          </div>
                          <span className="text-[9px] text-white/75">No cover art yet</span>
                        </div>
                      ) : (
                        <>
