...
import { Search, Plus, Edit, Copy, Upload, Globe, Tag, Loader2 } from "lucide-react";
import Link from "next/link";
import { CatalogingCopilotPanel } from "@/components/ai/CatalogingCopilotPanel";
      ...
      <PageContent className="p-0">
        <div className="h-full flex flex-col -m-6">
          ...
          <div className="flex-1 overflow-auto flex">
                ...
                <Table>
                  ...
                  <TableBody>
                    {records.map((record) => (
                      <TableRow
                        key={record.id}
                        className={`cursor-pointer ${selectedRecord?.id === record.id ? "bg-muted/50" : ""}`}
                        onClick={() => setSelectedRecord(record)}
                      >
                        <TableCell className="font-mono text-sm">{record.id}</TableCell>
                        <TableCell>
                          <Link
                            href={`/staff/cataloging/marc-editor?id=${record.id}`}
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            >
                              <Link
                                href={`/staff/cataloging/holdings?id=${record.id}`}
                                onClick={(e) => e.stopPropagation()}
                              >
            ...
            {featureFlags.ai && selectedRecord && (
              <div className="w-80 border-l overflow-auto p-3 shrink-0">
                <CatalogingCopilotPanel
                  marcData={{
                    title: selectedRecord.title,
                    author: selectedRecord.author || undefined,
                    isbn: selectedRecord.isbn || undefined,
                    publisher: selectedRecord.publisher || undefined,
                    physicalDescription: selectedRecord.physical_description || undefined,
                  }}
                  bibId={typeof selectedRecord.id === "number" ? selectedRecord.id : undefined}
                />
              </div>
            )}
          </div>
