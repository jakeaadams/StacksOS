...
const AVAILABLE_STATUS_IDS = new Set([0, 7]);
...
function HoldingsContent() {
  ...
  const isHoldingAvailable = (holding: HoldingRecord) => {
    if (AVAILABLE_STATUS_IDS.has(holding.statusId)) return true;
    const statusLower = holding.status.toLowerCase();
    return statusLower.includes("available") || statusLower.includes("reshelving");
  };

  const getStatusBadge = (holding: HoldingRecord) => {
    const statusLower = holding.status.toLowerCase();
    let className = "bg-muted/50 text-foreground";

    if (isHoldingAvailable(holding)) {
    ...
    return <Badge className={className}>{holding.status.toUpperCase()}</Badge>;
  };

  const availableCount = holdings.filter(isHoldingAvailable).length;
        ...
        <div className="h-full flex flex-col -m-6">
          <div className="bg-muted/50 border-b px-4 py-2 flex items-center gap-2">
            ...
            <Button asChild size="sm" variant="outline">
              <Link href="/staff/cataloging">Back to Cataloging</Link>
            </Button>
          </div>
            ...
            <div className="bg-background border-b px-4 py-3">
              <div className="flex items-start gap-4">
                ...
                <div className="text-right text-sm">
                  <div>
                    Bib ID: <span className="font-mono">{bibRecord.id}</span>
                  </div>
                  {bibRecord.isbn && (
                    <div>
                      ISBN: <span className="font-mono">{bibRecord.isbn}</span>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <TableBody>
                      {holdings.map((holding) => (
                        <TableRow key={holding.id}>
                          ...
                          <TableCell>{getStatusBadge(holding)}</TableCell>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div>{getStatusBadge(selectedHolding)}</div>
                    </div>
                  </div>
                  <div>
                    <label htmlFor="call-number" className="text-sm font-medium">
                      Call Number
                    </label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label htmlFor="circulation-count" className="text-sm font-medium">
                        Circulation Count
                      </label>
