...
/**
 * Moonshot/Kimi provider (OpenAI-compatible Chat Completions API).
 *
 * Env vars:
 *   MOONSHOT_API_KEY       — Bearer token
 *   MOONSHOT_API_BASE_URL  — defaults to NVIDIA NIM OpenAI-compatible endpoint
 */
...
async function moonshotChatCompletion(args: {
  ...
  if (!key) {
    throw new Error("Moonshot is not configured (missing MOONSHOT_API_KEY)");
  }
  ...
  try {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Moonshot HTTP ${res.status}: ${rawText.slice(0, 200)}`);
      }
    }
    if (!res.ok) {
      const msg = json?.error?.message || `Moonshot HTTP ${res.status}`;
      throw new Error(msg);
    }

    const text = json?.choices?.[0]?.message?.content;
    if (typeof text !== "string" || !text.trim()) {
      throw new Error("Moonshot returned an empty completion");
    }
