...
if ! curl -fsS --connect-timeout 3 --max-time 10 "$BASE_URL/api/health" >/dev/null 2>&1; then
...
if ! curl -fsS --connect-timeout 3 --max-time 10 "$BASE_URL/api/evergreen/ping" >/dev/null 2>&1; then
